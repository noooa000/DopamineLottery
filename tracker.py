...
# ---------------- main loop ----------------
# ★ CHANGED: accept rolls_per_multi and honor it for celebrations
def track_process(
    target_process,
    time_label,
    is_paused_func,
    ...
    time_required=None,
    stop_event=None,
    *,                                    # ★ NEW: force keyword-only for extras
    rolls_per_multi: int = 10             # ★ NEW: dynamic multi threshold (>=1)
):
    import threading
    ...
    try:
        rolls_per_multi = max(1, int(rolls_per_multi))  # ★ NEW: clamp
    except Exception:
        rolls_per_multi = 10

    tracked_time = _load_progress(target_process)
    total_tracked_time = tracked_time  # ← start UI timer from saved progress

    # push the initial value to the label immediately
    ...
    while not stop_event.is_set():    # loop exits on stop
        ...
        # robust process name match (case-insensitive)
        try:
            running = any(
                (p.info.get('name') or '').lower() == target_process.lower()
        ...
        if running:
            ...
            while tracked_time >= time_required:
                ...
                total = load_chances()
                # ★ CHANGED: celebrate on every multiple of rolls_per_multi
                if total % rolls_per_multi == 0:
                    time_label.after(0, lambda: time_label.config(text=f"🎉 {rolls_per_multi} chances reached!"))
                    cheer_callback()
                else:
                    _play_cat_sound()
